    ...
    <div className={style.profile_container}>
      <div className={style._header}>
        ...
        {props.currentUser.idUrl === props.user.idUrl ? (
          <Link to={`/profile/${props.currentUser.idUrl}/settings`}>
            <button className={style.btn_setting}>{data.btnSettings}</button>
          </Link>
        ) : (
          ""
        )}
      ...
      <div className={style.add_recipe}>
        {props.currentUser.idUrl === props.user.idUrl ? (
          <Link to={`/profile/add_recipe`}>
            <button>{data.btnNameAddREcipe}</button>
          </Link>
        ) : (
          ""
        )}
      ...
      {props.currentUser.idUrl === props.user.idUrl ? (
        <div className={style.tabs}>
          <button onClick={(e) => props.btnTabs(e.target.textContent)}>
            {data.btnsTabs.tabName1}
          </button>
          <button onClick={(e) => props.btnTabs(e.target.textContent)}>
            {data.btnsTabs.tabName2}
          </button>
          <button onClick={(e) => props.btnTabs(e.target.textContent)}>
            {data.btnsTabs.tabName3}
          </button>
        </div>
      ) : (
        <div className={style.tabs}>
