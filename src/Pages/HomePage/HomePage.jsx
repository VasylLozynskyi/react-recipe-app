...
export const HomePage = (props) => {
  ...
  let recipes_user =
    recipes.length === 0 ? (
      ...
      recipes.map((card) => <CardRecipeHomePage key={card.id} data={card} />)
    ) : (
      ""
    );
  let newRecipes = recipes
    ? recipes
        .sort((a, b) => Date.parse(b.timeAdd) - Date.parse(a.timeAdd))
        .slice(0, 3)
    : "";
  ...
  return (
    <div className={style.wrapper_UserHomePage}>
      ...
      <div className={style.container_filter} onClick={handleCategory}>
        {Categories.map((btn) => (
          <ButtonCategories key={btn} category={btn} />
        ))}
      </div>
      <div className={style.recipes_home_container}>
        <div className={style.carousel_recipes_user}>{recipes_user}</div>
      </div>
